...
public class CrazyPermission {
    ...
    public static void cameraAndStorage(Context context, IPermissionResult iPermissionResult) {
        XXPermissions.with(context)
                .permissions(
                        Arrays.asList(new StandardDangerousPermission(Manifest.permission.CAMERA,33),
                                new StandardDangerousPermission(Manifest.permission.READ_EXTERNAL_STORAGE,33),
                                new StandardDangerousPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE,33))
                )
                .request(new OnPermissionCallback() {
                    @Override
                    public void onResult(@NonNull List<IPermission> grantedList, @NonNull List<IPermission> deniedList) {
